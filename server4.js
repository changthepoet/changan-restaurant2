...
server.on('request', function(request, response) { // 当有request请求的时候触发处理函数
    ...
    case ''||'/' : // 模拟欢迎页,nodejs是高效流处理的方案,也可以通过配置文件来配置
        ...
        case '/bincode.html':
            fs.readFile('./bincode.html', function(err, content){
                response.writeHead(200, { 'Content-Type' : 'text/html; charset=UTF-8' });
                response.write(content);
                var a="";
                var newstr = "";
                ...
                var str = JSON.stringify(b,null,2);
                a = str;
                var data = fs.readFileSync('mingdan.json',"utf-8");
                var d = JSON.parse(data);
                newdata = JSON.stringify(d,null,2); 
                newstr = ','+a+']';
                console.log(newstr);
                nowstr+= newdata.replace(/]/,newstr);
                console.log(nowstr);
                fs.writeFile('mingdan.json',nowstr,function(err){
                    if(err) throw err;
                    console.log('write JSON into mingdan.json');
                    });
                });
                response.end();
            });
